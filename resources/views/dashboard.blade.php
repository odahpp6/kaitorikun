...
<h2 class="text-xl">{{ $currentMonthLabel ?? now()->format('Y年n月') }}の売り上と買取金額</h2>


<div class="grid grid-cols-1 md:grid-cols-2 gap-4 my-4">
    <div class="p-4 bg-blue-100 rounded-lg text-center">
        <h3 class="text-lg font-semibold mb-2">売上金額</h3>
        <p class="text-2xl font-bold text-blue-600">{{ number_format($salesTotal ?? 0) }}</p>
    </div>
    <div class="p-4 bg-green-100 rounded-lg text-center">
        <h3 class="text-lg font-semibold mb-2">買取金額</h3>
        <p class="text-2xl font-bold text-green-600">{{ number_format($purchaseTotal ?? 0) }}</p>
    </div>
</div>
...
<div class="flex flex-wrap mt-4">
  <div class="md:w-1/4 w-full p-2">
    <a href="{{ route('estimate.register') }}" class="text-blue rounded-lg bg-blue-100 p-4 block mb-4 text-center">
      <i class="fa-solid fa-file-pen w-8 h-8 text-blue-600 text-2xl"></i>
      <span class="text-blue-600">見積登録</span>
    </a>
  </div>
<div class="md:w-1/4 w-full p-2">
    <a href="{{ route('purchase.register') }}" class="text-blue rounded-lg bg-green-100 p-4 block mb-4 text-center">
 ...
 <div class="md:w-1/4 w-full p-2">
  ...
  <div class="md:w-1/4 w-full p-2">
    <a href="{{ route('cash_management.view') }}" class="text-blue rounded-lg bg-purple-100 p-4 block mb-4 text-center">
      <i class="fa-solid fa-clock-rotate-left w-8 h-8 text-2xl text-purple-600"></i>
